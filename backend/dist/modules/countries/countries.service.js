...
const CONTINENT_BY_CODE = Object.entries(CONTINENT_CODES).reduce((acc, [continent, codes]) => {
...
export const getCountries = async (query = {}) => {
    ...
    if (search) {
        countries = countries.filter((country) => country.name.toLowerCase().includes(search) ||
