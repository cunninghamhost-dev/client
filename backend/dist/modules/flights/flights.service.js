 ...
 */
const tiqwaFetch = async (endpoint, options = {}) => {
    // Ensure we don't have double slashes or missing slashes
    const baseUrl = BASE_URL?.endsWith('/') ? BASE_URL.slice(0, -1) : BASE_URL;
    const cleanEndpoint = endpoint.startsWith('/') ? endpoint : `/${endpoint}`;
    if (!baseUrl)
        throw new Error("TIQWA_BASE_URL is not defined in .env");
    if (!API_KEY)
 ...
 */
// backend/src/modules/flights/flights.service.ts
export const confirmPrice = async (flightId) => {
 ...
 * 🧾 Book Flight
 */
export const bookFlight = async (flight_id, payload) => {
 ...
 * 📄 Booking Details
 */
export const getBookingDetails = async (reference) => {
 ...
 */
export const issueTicket = async (reference) => {
