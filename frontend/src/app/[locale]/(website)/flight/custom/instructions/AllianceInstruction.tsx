...
const alliances = Array.from({ length: 3 }, (_, i) => ({
  id: i,
  title: `Star Alliance ${i + 1}`,
  amount: 'US$8,992',
  image_reference: '/images/main/star-alliance.png',
}));

const formSchema = z.object({
  alliances: z.array(z.number()).refine((value) => value.some((item) => item), {
    message: 'You have to select at least one item.',
  }),
});

const AllianceInstruction = () => {
  ...
  return (
    <div className='mt-0'>
      <Card className='w-full bg-[#edeff1]  rounded-none border-0 border-t-[1px] border-t-gray-500 shadow-none px-2'>
        <CardTitle>
          <h5 className='font-bold text-[#0F294D] text-[13.89px] leading-[18px]'>Alliance</h5>
        </CardTitle>
...
export default AllianceInstruction;
