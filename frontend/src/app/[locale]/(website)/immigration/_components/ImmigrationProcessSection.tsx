...
import { ConstImmigrationProcesses as processes } from '@/lib/constants/website/visa-assistance/visa.constant';
import { IVisaProcessProps } from '@/types/website/visa-assistance.type';
import { ChevronLeft, ChevronRight } from 'lucide-react';
import Image from 'next/image';

const ImmigrationProcessSection = () => {
      ...
      <div className='container mx-auto px-4'>
        <div className='text-center mb-12'>
          ...
          <p className='text-[#666666] mt-2 max-w-xl md:max-w-3xl mx-auto text-base md:text-lg lg:text-xl leading-[150%]'>
            Learn more about the process of each kind of Immigration we can help you apply for
          </p>
        </div>
        <Tabs value={activeTab} onValueChange={setActiveTab} className='w-full'>
          <div className='flex items-center justify-center relative border-b border-gray-300'>
            ...
            <TabsList
              ref={tabsListRef}
              className='flex flex-nowrap justify-start h-auto bg-transparent p-0 overflow-x-hidden scrollbar-hide w-full max-w-155 mx-auto'
            >
              ...
              <div className='mt-8 grid grid-cols-1 md:grid-cols-2 gap-12 items-center'>
                ...
                <div className='space-y-8 px-12'>
                  {process.steps.map((step) => (
                    <div key={step.id} className='flex items-start space-x-4'>
                      <div className='shrink-0 flex items-center justify-center w-16 h-16 rounded-full bg-[#E6F5F9] text-xl md:text-2xl lg:text-3xl'>
                        {step.id}
                      </div>
                  ...
                  <div className='pt-4 px-4'>
                    <Button className='bg-[#E63A24] hover:bg-orange-700 rounded-xl py-3 px-5 font-semibold cursor-pointer'>
                      Learn More
                    </Button>
                  </div>
                </div>
                ...
                <div className='hidden md:block'>
                  <Image
                    ...
                    width={441}
                    height={562}
                    className='rounded-lg object-cover w-full h-full max-h-140.5'
                  />
                </div>
              </div>
            </TabsContent>
...
export default ImmigrationProcessSection;
